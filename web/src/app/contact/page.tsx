import { Metadata } from "next";
import { Header } from "@/components/layout/Header";
import { Footer } from "@/components/layout/Footer";
import { Container } from "@/components/ui/Container";
import { ContactForm } from "@/components/forms/ContactForm";

export const metadata: Metadata = {
  title: "Contact | Trend Wood",
  description:
    "Contactați-ne pentru mobilier din lemn masiv la comandă. Telefon, WhatsApp sau formular de contact.",
};

export default function Contact() {
  return (
    <>
      <Header />

      <main className="pt-20 md:pt-24">
        {/* Page Hero */}
        <section className="py-24 md:py-32 bg-hover">
          <Container>
            <div className="max-w-3xl">
              <h1 className="font-display text-h1 text-charcoal mb-6">
                Hai să vorbim
              </h1>
              <p className="text-stone text-body-lg">
                Suntem aici pentru a transforma ideile dumneavoastră în mobilier
                care va dura generații.
              </p>
            </div>
          </Container>
        </section>

        {/* Contact Content */}
        <section className="py-24 md:py-32">
          <Container>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
              {/* Contact Info */}
              <div>
                <h2 className="font-display text-h2 text-charcoal mb-8">
                  Informații de contact
                </h2>

                <div className="space-y-8">
                  {/* Phone */}
                  <div>
                    <h3 className="font-display text-lg text-charcoal mb-2">
                      Telefon
                    </h3>
                    <div className="space-y-1">
                      <a
                        href="tel:+40724533675"
                        className="block text-stone hover:text-forest transition-colors"
                      >
                        0724 533 675
                      </a>
                      <a
                        href="tel:+40731491811"
                        className="block text-stone hover:text-forest transition-colors"
                      >
                        0731 491 811
                      </a>
                    </div>
                  </div>

                  {/* WhatsApp */}
                  <div>
                    <h3 className="font-display text-lg text-charcoal mb-2">
                      WhatsApp
                    </h3>
                    <a
                      href="https://wa.me/40724533675"
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-2 text-forest hover:text-moss transition-colors"
                    >
                      <svg
                        className="w-5 h-5"
                        fill="currentColor"
                        viewBox="0 0 24 24"
                      >
                        <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                      </svg>
                      Scrie-ne pe WhatsApp
                    </a>
                  </div>

                  {/* Email */}
                  <div>
                    <h3 className="font-display text-lg text-charcoal mb-2">
                      Email
                    </h3>
                    <a
                      href="mailto:trendwoodconsult@gmail.com"
                      className="text-stone hover:text-forest transition-colors"
                    >
                      trendwoodconsult@gmail.com
                    </a>
                  </div>

                  {/* Location */}
                  <div>
                    <h3 className="font-display text-lg text-charcoal mb-2">
                      Locație
                    </h3>
                    <p className="text-stone">Ciolpani, Ilfov</p>
                  </div>
                </div>
              </div>

              {/* Contact Form */}
              <div>
                <h2 className="font-display text-h2 text-charcoal mb-8">
                  Trimite un mesaj
                </h2>
                <ContactForm />
              </div>
            </div>
          </Container>
        </section>
      </main>

      <Footer />
    </>
  );
}
