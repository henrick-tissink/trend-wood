import { getTranslations } from 'next-intl/server'
import { Header } from '@/components/layout/Header'
import { Footer } from '@/components/layout/Footer'
import { Container } from '@/components/ui/Container'
import { ContactForm } from '@/components/forms/ContactForm'
import { FadeIn } from '@/components/ui/FadeIn'
import type { Locale } from '@/lib/i18n/config'

type Props = {
  locale: Locale
}

export async function ContactPage({ locale }: Props) {
  const t = await getTranslations({ locale, namespace: 'contact' })

  return (
    <>
      <Header locale={locale} />

      <main className="pt-20 md:pt-24">
        {/* Page Hero */}
        <section className="py-section md:py-section-lg bg-hover">
          <Container>
            <div className="max-w-3xl">
              <FadeIn>
                <h1 className="font-display text-h1 text-charcoal mb-6">
                  {t('hero.title')}
                </h1>
              </FadeIn>
              <FadeIn delay={0.1}>
                <p className="text-stone text-body-lg max-w-xl">
                  {t('hero.subtitle')}
                </p>
              </FadeIn>
            </div>
          </Container>
        </section>

        {/* Contact Content */}
        <section className="py-section-lg md:py-section-xl bg-cream">
          <Container>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24">
              {/* Contact Info */}
              <div>
                <FadeIn>
                  <h2 className="font-display text-h2 text-charcoal mb-6">
                    {t('info.title')}
                  </h2>
                </FadeIn>
                <FadeIn delay={0.1}>
                  <p className="text-stone text-body-lg mb-12">
                    {locale === 'ro'
                      ? 'Preferați să discutăm la telefon? Sunați-ne direct sau scrieți-ne pe WhatsApp. Răspundem în cel mai scurt timp.'
                      : locale === 'en'
                      ? 'Prefer to talk on the phone? Call us directly or write to us on WhatsApp. We respond as soon as possible.'
                      : 'Bevorzugen Sie ein Telefongespräch? Rufen Sie uns direkt an oder schreiben Sie uns auf WhatsApp. Wir antworten schnellstmöglich.'
                    }
                  </p>
                </FadeIn>

                <div className="space-y-8">
                  {/* Phone */}
                  <FadeIn delay={0.15}>
                    <div className="flex items-start gap-5">
                      <div className="w-12 h-12 rounded-full bg-forest/10 flex items-center justify-center flex-shrink-0">
                        <svg
                          className="w-5 h-5 text-forest"
                          fill="none"
                          stroke="currentColor"
                          viewBox="0 0 24 24"
                        >
                          <path
                            strokeLinecap="round"
                            strokeLinejoin="round"
                            strokeWidth={1.5}
                            d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"
                          />
                        </svg>
                      </div>
                      <div>
                        <h3 className="font-display text-h4 text-charcoal mb-2">
                          {t('info.phone')}
                        </h3>
                        <div className="space-y-1">
                          <a
                            href="tel:+40731491811"
                            className="block text-stone text-body-lg hover:text-forest transition-colors duration-fast"
                          >
                            0731 491 811 - Utu
                          </a>
                          <a
                            href="tel:+40724533675"
                            className="block text-stone text-body-lg hover:text-forest transition-colors duration-fast"
                          >
                            0724 533 675 - Ioana
                          </a>
                        </div>
                      </div>
                    </div>
                  </FadeIn>

                  {/* WhatsApp */}
                  <FadeIn delay={0.2}>
                    <div className="flex items-start gap-5">
                      <div className="w-12 h-12 rounded-full bg-forest/10 flex items-center justify-center flex-shrink-0">
                        <svg
                          className="w-5 h-5 text-forest"
                          fill="currentColor"
                          viewBox="0 0 24 24"
                        >
                          <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                        </svg>
                      </div>
                      <div>
                        <h3 className="font-display text-h4 text-charcoal mb-2">
                          WhatsApp
                        </h3>
                        <a
                          href="https://wa.me/40724533675"
                          target="_blank"
                          rel="noopener noreferrer"
                          className="inline-flex items-center gap-2 text-forest text-body-lg hover:text-moss transition-colors duration-fast"
                        >
                          {locale === 'ro'
                            ? 'Scrie-ne pe WhatsApp'
                            : locale === 'en'
                            ? 'Message us on WhatsApp'
                            : 'Schreiben Sie uns auf WhatsApp'
                          }
                          <svg
                            className="w-4 h-4"
                            fill="none"
                            stroke="currentColor"
                            viewBox="0 0 24 24"
                          >
                            <path
                              strokeLinecap="round"
                              strokeLinejoin="round"
                              strokeWidth={1.5}
                              d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"
                            />
                          </svg>
                        </a>
                      </div>
                    </div>
                  </FadeIn>

                  {/* Email */}
                  <FadeIn delay={0.25}>
                    <div className="flex items-start gap-5">
                      <div className="w-12 h-12 rounded-full bg-forest/10 flex items-center justify-center flex-shrink-0">
                        <svg
                          className="w-5 h-5 text-forest"
                          fill="none"
                          stroke="currentColor"
                          viewBox="0 0 24 24"
                        >
                          <path
                            strokeLinecap="round"
                            strokeLinejoin="round"
                            strokeWidth={1.5}
                            d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"
                          />
                        </svg>
                      </div>
                      <div>
                        <h3 className="font-display text-h4 text-charcoal mb-2">
                          {t('info.email')}
                        </h3>
                        <a
                          href="mailto:trendwoodconsult@gmail.com"
                          className="text-stone text-body-lg hover:text-forest transition-colors duration-fast"
                        >
                          trendwoodconsult@gmail.com
                        </a>
                      </div>
                    </div>
                  </FadeIn>

                  {/* Location */}
                  <FadeIn delay={0.3}>
                    <div className="flex items-start gap-5">
                      <div className="w-12 h-12 rounded-full bg-forest/10 flex items-center justify-center flex-shrink-0">
                        <svg
                          className="w-5 h-5 text-forest"
                          fill="none"
                          stroke="currentColor"
                          viewBox="0 0 24 24"
                        >
                          <path
                            strokeLinecap="round"
                            strokeLinejoin="round"
                            strokeWidth={1.5}
                            d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"
                          />
                          <path
                            strokeLinecap="round"
                            strokeLinejoin="round"
                            strokeWidth={1.5}
                            d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"
                          />
                        </svg>
                      </div>
                      <div>
                        <h3 className="font-display text-h4 text-charcoal mb-2">
                          {t('info.location')}
                        </h3>
                        <p className="text-stone text-body-lg">
                          Ciolpani, Ilfov{locale !== 'ro' ? ', Romania' : ''}
                        </p>
                        <p className="text-stone/60 text-small mt-1">
                          {locale === 'ro'
                            ? 'Venim la dumneavoastră pentru măsurători'
                            : locale === 'en'
                            ? 'We come to you for measurements'
                            : 'Wir kommen zu Ihnen für Messungen'
                          }
                        </p>
                      </div>
                    </div>
                  </FadeIn>
                </div>
              </div>

              {/* Contact Form */}
              <div>
                <FadeIn delay={0.1}>
                  <div className="bg-hover p-8 md:p-10 lg:p-12">
                    <h2 className="font-display text-h2 text-charcoal mb-3">
                      {locale === 'ro'
                        ? 'Trimite un mesaj'
                        : locale === 'en'
                        ? 'Send a message'
                        : 'Senden Sie eine Nachricht'
                      }
                    </h2>
                    <p className="text-stone mb-10">
                      {locale === 'ro'
                        ? 'Completează formularul și te contactăm în cel mai scurt timp.'
                        : locale === 'en'
                        ? 'Fill out the form and we will contact you as soon as possible.'
                        : 'Füllen Sie das Formular aus und wir werden Sie so schnell wie möglich kontaktieren.'
                      }
                    </p>
                    <ContactForm />
                  </div>
                </FadeIn>
              </div>
            </div>
          </Container>
        </section>
      </main>

      <Footer locale={locale} />
    </>
  )
}
